...
def problem1():
    ...
    if check_func(trends.tweet_text, zip(tweets_as_dicts, expected_texts)):
        ...
        location_tuple(trends.tweet_location), zip(tweets_as_dicts, expected_locations)
    ):
        return True
    ...
    if check_func(trends.tweet_text_fn, zip(tweets_as_fns, expected_texts)):
        ...
        location_tuple(trends.tweet_location_fn), zip(tweets_as_fns, expected_locations)
    ):
        return True
...
def problem2():
    ...
    for i in range(32, 128):
        c = chr(i)
        if c in string.ascii_letters or not c in string.printable:
            continue
...
def problem4():
    ...
    sentiment_tests = (
        (
            (
                trends.make_tweet(
                    "Help, I'm trapped in an autograder factory and I can't get out!".lower(),
                    None,
                    0,
                    0,
                ),
        ...
        (
            (
                trends.make_tweet(
                    "The thing that I love about hating things that I love is that I hate loving that I hate doing it.".lower(),
                    None,
                    0,
                    0,
                ),
    ...
    no_sentiment_tests = (
        (
            (
                trends.make_tweet(
                    "Peter Piper picked a peck of pickled peppers".lower(), None, 0, 0
                ),
            ),
            None,
        ),
...
def problem5():
    ...
    try:
        ...
        trends.make_position = geo.make_position = lambda lat, long: (
            lambda z: z * lat + (1 - z) * long
        )
...
def problem8():
    def test_average():
        tweets = pirate_tweets(trends.make_tweet) + (
            ...
            trends.make_tweet(
                "This tweet is also without a sentiment", None, None, None
            ),
        )
        tweets_by_state = {
            "MT": [tweets[1], tweets[5]],
...
def tricky_tweets(make_tweet):
    return (
        make_tweet(
            "Hello, world! !dlrow ,olleH".lower(),
            datetime.strptime("2010-01-02 01:04:05", "%Y-%m-%d %H:%M:%S"),
            38,
            -120,
        ),
        make_tweet(
            "I mean what person wouldn't? It's John Stamos!".lower(),
            datetime.strptime("2010-01-02 02:04:05", "%Y-%m-%d %H:%M:%S"),
            38,
            -119,
        ),
        make_tweet(
            "Ph'nglui mglw'nafh Cthulhu R'lyeh wgah'nagl fhtagn".lower(),
            datetime.strptime("2010-01-02 03:04:05", "%Y-%m-%d %H:%M:%S"),
            38,
            -118,
        ),
        make_tweet(
            "kfdafj. afjdsal.jfdlsafj. fjd jksa".lower(),
            datetime.strptime("2010-01-02 04:04:05", "%Y-%m-%d %H:%M:%S"),
            38,
            -117,
        ),
        make_tweet(
            "/Never/look up.when dragons!fly overhead.".lower(),
            datetime.strptime("2010-01-02 05:04:05", "%Y-%m-%d %H:%M:%S"),
            38,
            -116,
        ),
        make_tweet(
            "Jeepers, what fools these mortals be!".lower(),
...
def pirate_tweets(make_tweet):
    return (
        make_tweet(
            "I am the very model of a modern Major-General".lower(), None, 43, -84
        ),
        make_tweet(
            "I've information vegetable, animal, and mineral".lower(), None, 58, -112
        ),
        make_tweet(
            "I know the kings of England, and I quote the fights historical".lower(),
            None,
            49,
            -104,
        ),
        make_tweet(
            "From Marathon to Waterloo, in order categorical".lower(), None, 19, -87
        ),
        make_tweet(
            "I'm very well acquainted, too, with matters mathematical".lower(),
            None,
            44,
            -85,
        ),
        make_tweet(
            "I understand equations, both the simple and quadratical".lower(),
            None,
            59,
            -110,
        ),
        ...
        make_tweet(
            "With many cheerful facts about the square of the hypotenuse".lower(),
            None,
            15,
            -87,
        ),
